...
func GetCardLogicValue(cbCardData byte) int {
	...
	bCardValue := games.GetCardValue(cbCardData)

	//转换数值
	return games.If(int(bCardValue) > 10, 10, int(bCardValue)).(int)
...
func GetUnicode(cbCardData [5]byte, cbCardCount byte) {
	...
	for i := 0; i < len(cbCardData); i++ {
		...
		switch games.GetCardColor(cbCardData[i]) {
		case 0x00:
			str += " 方块"
			break
		case 0x10:
		...
		switch games.GetCardValue(cbCardData[i]) {
		case 1:
			str += "A"
			break
		case 2:
...
func GetCardType(cbCardData [MAX_COUNT]byte, cbCardCount int) int {
	...
	for i := 0; i < cbCardCount; i++ {
		...
		if games.GetCardValue(cbCardData[i]) > 10 {
			bKingCount++
		} else if games.GetCardValue(cbCardData[i]) == 10 {
			bTenCount++
		}
	...
	if bSmallWang || bBigWang {
		//大小王都有，那就只要判断有两张一样的牌
		if bSmallWang && bBigWang {
			if games.GetCardValue(bFirstTemp[2]) == games.GetCardValue(bFirstTemp[3]) || games.GetCardValue(bFirstTemp[3]) == games.GetCardValue(bFirstTemp[4]) {
				return OX_FOUR_SAME
			}
		} else ////只有一个大小王，那就判断三张一样的牌
		{
			if games.GetCardValue(bFirstTemp[1]) == games.GetCardValue(bFirstTemp[3]) || games.GetCardValue(bFirstTemp[2]) == games.GetCardValue(bFirstTemp[4]) {
				return OX_FOUR_SAME
			}
		}
	} else //没有大小王的情况，判断四张
	{
		if games.GetCardValue(bFirstTemp[0]) == games.GetCardValue(bFirstTemp[3]) || games.GetCardValue(bFirstTemp[1]) == games.GetCardValue(bFirstTemp[4]) {
			return OX_FOUR_SAME
		}
	}
	...
	if bSmallWang || bBigWang {
		...
		for i := 0; i < cbCardCount; i++ {
			...
			for j := 0; j < cbCardCount; j++ {
				...
				cbTemp = games.If(cbTempValue[0]+cbTempValue[1] > 10, cbTempValue[0]+cbTempValue[1]-10, cbTempValue[0]+cbTempValue[1]).(int)
			...
			for j := 0; j < cbCardCount; j++ {
				...
				for k := 0; k < cbCardCount; k++ {
					...
					if (cbTempValue[0]+cbTempValue[1]+cbTempValue[2])%10 == 0 {
						return games.If(bSmallWang == true, OX_DOUBLECOW, OX_DOUBLECOW).(int)
	...
	for i := 0; i < cbCardCount-1; i++ {
		...
		for j := i + 1; j < cbCardCount; j++ {
			...
			if (bSum-bTemp[i]-bTemp[j])%10 == 0 {
				...
				cbTempValue = games.If((bTemp[i]+bTemp[j]) > 10, bTemp[i]+bTemp[j]-10, bTemp[i]+bTemp[j]).(int)
				if cbTempValue == 10 {
					return OX_DOUBLECOW
				} else {
					return cbTempValue
...
func CompareCard(cbFirstData [MAX_COUNT]byte, cbNextData [MAX_COUNT]byte, cbCardCount int) bool {
	...
	// 	//比较数值
	cbNextMaxValue := games.GetCardValue(bNextTemp[0])
	cbFirstMaxValue := games.GetCardValue(bFirstTemp[0])
	var cbPosFirst, cbPosNext byte
	if cbNextMaxValue != cbFirstMaxValue {
		return cbFirstMaxValue > cbNextMaxValue
	}
	// 	//比较颜色
	return games.GetCardColor(bFirstTemp[cbPosFirst]) > games.GetCardColor(bNextTemp[cbPosNext])
...
func SortCardList(cbCardData *[5]byte, cbCardCount int) {
	...
	for i := 0; i < cbCardCount; i++ {
		cbLogicValue[i] = games.GetCardValue(cbCardData[i])
	}
